...
const BlogLists = () => {
  ...
  const [allListedBlogs, setAllListedBlogs] = useState([]);
  ...
  const fetchAllBlogs = async () => {
    ...
    console.log(res.data[0].media[0].url);

    setAllListedBlogs(res.data);
  };

  const updateBlogs = async (blog) => {
    console.log(blog, "blog");

    const response = await fetch(`/api/blog/update/${blog._id}`, {
      method: "PUT",
  ...
  const filterEvents = (e) => {
    ...
    for (const event of allListedBlogs) {
      if (event?.title.toLowerCase().includes(blogName.toLowerCase())) {
        tempArray.push(event);
      }
    }

    setAllFilteredBlogs(tempArray);
  };
  ...
  const displayBlogsList = (blog) => {
    return (
      <div className="grid grid-cols-4 gap-4 border-b-2">
        ...
        <div className="w-full py-6">{blog.title}</div>
        <div className="w-full py-6">
          {blog.isPublished ? (
            <Tooltip
              rounded
  ...
  return (
    <div className="w-[58rem] pt-[1.5rem] mx-auto w-full ">
      <h2 className="text-lg font-bold mb-4">Listed Blogs</h2>
      <div className="bg-white p-6 rounded-lg shadow-md">
        <div className="flex justify-between">
          ...
          <div className="justify-center">
            <button onClick={() => router.push({ pathname: "createBlog" })}>
              <a
        ...
        {allFilteredBlogs && allFilteredBlogs.length > 0 ? (
          allFilteredBlogs.map((blog) => displayBlogsList(blog))
        ) : allListedBlogs && allListedBlogs.length > 0 ? (
          allListedBlogs.map((blog) => displayBlogsList(blog))
        ) : (
          <div></div>
        )}
      </div>
